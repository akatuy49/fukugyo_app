...
# 質問7（追加）：悩み・不安
q7 = st.text_area("Q7. 今抱えている悩みや不安（収入・時間・スキルなど）を教えてください", "")
...
if st.button("診断する"):
    if "選択してください" in [q1, q3, q5, q6] or not q2 or not q4:
        ...
        prompt = f"""
あなたは副業診断の専門家で副業コンサルタントです。

以下のユーザー情報をもとに、その人に合った副業のタイプを自然な日本語で提案してください。
ユーザーの悩みやインサイトまでを深掘りして、詳細な副業と戦略を提案してください。
具体的なロードマップまで提案してください。
必要な投資（有料教材、コンサル）などの提案をしてください。
診断結果には以下の内容を含めてください：
- タイプ名（例：デザイナー、SNSクリエイター型など）
- 特徴（なぜそのタイプに当てはまるか）
- おすすめ副業ジャンル（3つ程度）
- 具体的なロードマップ
- 具体的な戦略
